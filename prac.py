...
def show_history(column_name,table_name):
    ...
    Label(hist_window, text='Search by:', font='Courier').grid(row=0,column=0,padx=4,pady=3,sticky=tk.E)
    ...
    Label(hist_window, text='Minimum Value :', font='Courier').grid(row=1,column=0,padx=4,pady=3,sticky=tk.E)
    value=IntVar(hist_window)
    value.set(0)
    Entry(hist_window, textvariable=value).grid(row=1, column=1,padx=20,pady=15,sticky=tk.W)
    button = tk.Button(hist_window, bg='LightSlateGray', command=lambda :show_table(hist_window,table_name,val,value,column_name), font='Courier',
    ...
    button1 = tk.Button(hist_window, bg='LightSlateGray',
                       command=lambda: delete_all_data(table_name), font='Courier',
    ...
    if(len(column_name)==7):
        tv = ttk.Treeview(frn, columns=(1, 2, 3, 4, 5, 6, 7), show='headings', height=10)
        tv.pack()
    elif(len(column_name)==4):
        tv = ttk.Treeview(frn, columns=(1, 2, 3, 4), show='headings', height=10)
        tv.pack()
